...
class WyckoffStageFactor(Factor):
    ...
    def _judge_full(self, c, h, l, v, sub_events, window, period_label,
        ...
        # ---- 基础指标 ----
        ma20 = statistics.mean(c[-20:]) if len(c) >= 20 else 0
        ma50 = statistics.mean(c[-50:]) if len(c) >= 50 else ma20
        ma60 = statistics.mean(c[-60:]) if len(c) >= 60 else ma20
        ma200 = statistics.mean(c) if len(c) >= 200 else ma60
        ...
        # ---- MA 金叉 / 持续上方 ----
        ma50_prev = statistics.mean(c[-51:-1]) if len(c) >= 51 else ma50
        ma200_prev = statistics.mean(c[:-1]) if len(c) >= 201 else ma200
        golden_cross_recent = (ma50_prev <= ma200_prev and ma50 > ma200)
        confirm_days = 5
        above_ma200 = p > ma200
        if len(c) >= ma_long + confirm_days:
            ma200_series = [statistics.mean(c[max(0, i-ma_long):i+1]) for i in range(len(c))]
        ...
        # ---- 量能 ----
        vol_5 = statistics.mean(v[-5:]) if len(v) >= 5 else 0
        vol_60 = statistics.mean(v[-60:]) if len(v) >= 60 else 0
        vol_shrink = vol_5 < vol_60 * 0.5 if vol_60 > 0 else False
        dist_confirm_days = 3
        if len(v) >= 60 + dist_confirm_days:
            recent_vol_3d = statistics.mean(v[-dist_confirm_days:])
        ...
        if len(c) >= 30:
            ...
            if len(v) >= 120:
                vol_recent = statistics.mean(v[-20:])
                vol_ref = statistics.mean(v[-120:-20])
            elif len(v) >= 40:
                vol_recent = statistics.mean(v[-10:])
                vol_ref = statistics.mean(v[-30:-10])
            else:
        ...
        ma50_angle_5d = 0
        if len(c) >= 55:
            ma50_5d_ago = statistics.mean(c[-55:-50])
            ma50_now = statistics.mean(c[-50:])
            if ma50_5d_ago > 0:
