"""near_low_backtest.py — t-near-low 策略回测

策略定义 (跟 t-near-low 完全一致):
  1. 5y 内 max_dd 跌 70-80%   (在历史曾大跌)
  2. 距 5y low < gap%         (现在接近历史底部)
  3. 排除 ST, 市值 >= 50亿 (实战)

历史回测: 命中后 N 日 max / close 涨幅, 胜率/均涨幅统计
...
数据: K线从 DataStore 读, 5y low 用 weekly 算
"""
import argparse
import glob
import sqlite3
...
def find_max_drawdown(closes, dates):
    """找窗口内最深回撤 (high→low)
    Returns: (max_dd_pct, low_idx, high_idx)
    """
    if not closes:
...
def count_bounces(weekly_closes, threshold=0.30, window=3):
    """5y weekly 内 30%+ 反弹事件, window=3 strict local min"""
    n = len(weekly_closes)
    if n < window * 2:
        return 0
    cnt = 0
    for i in range(window, n - window):
        # strict local min in window
        is_min = all(weekly_closes[i] <= weekly_closes[i - j] for j in range(1, window + 1)) \
                 and all(weekly_closes[i] <= weekly_closes[i + j] for j in range(1, window + 1))
        if not is_min:
            continue
        # 之前 30%+ 高点
        future_max = max(weekly_closes[i + 1: i + window * 4 + 1]) if i + window * 4 + 1 <= n else max(weekly_closes[i + 1:])
        bounce = (future_max - weekly_closes[i]) / weekly_closes[i]
        if bounce >= threshold:
            cnt += 1
    return cnt


def scan_one(code: str, lookback_years: int, drop: float, drop_max: float, gap: float,
             take_profit: float, stop_loss: float, max_hold: int,
             min_mcap: float, exclude_st: bool, basic_map: dict) -> list:
    """扫单只票所有"距 5y low < gap%" 命中日, 返回每笔完整交易"""
    try:
        # 股票质量过滤
        if exclude_st and basic_map.get(code, {}).get("name"):
            if "ST" in basic_map[code].get("name", ""):
                return []
        mcap = basic_map.get(code, {}).get("market_cap", 0) or 0
        ...
        # 5y low (整段历史 weekly 最低)
        lo_5y = min(closes)  # 用 daily 最低更准

        # max_dd 窗口 (5y lookback)
        lookback_days = lookback_years * 252
        lb_closes = closes[-lookback_days:] if len(closes) > lookback_days else closes
        max_dd, _, _ = find_max_drawdown(lb_closes, None)
        if max_dd is None:
            return []
        if max_dd > -drop:
            return []  # 没跌够
        ...
        # 找所有"距 5y low < gap%" 的命中日
        # 限制在 lookback 窗口内 (跟回测时间范围一致)
        cutoff_date = (datetime.now() - timedelta(days=lookback_years * 365)).strftime("%Y%m%d")
        hits = []
        last_hit_date = None  # dedup: 同票 30 日内不重复
        for i in range(60, len(rows) - max_hold):
            d = dates[i]
            if d < cutoff_date:
                continue
            # dedup: 同票 30 日内只记第一笔 (避免反复触发)
            ...
            if cur <= lo_5y:
                continue
            gap_pct = (cur - lo_5y) / lo_5y
            if gap_pct >= gap / 100.0:
                continue
...
def _load_all_basic() -> dict:
    try:
        ...
        df = tbl.to_pandas()
        # 市值从 daily_basic 拉 (total_share 字段在 basic 里全是 0)
        # 1 次拉全市场 daily_basic 最新 1 条
        result = {}
        ...
        try:
            ...
            for _, r in db_latest.iterrows():
                ...
                total_share = float(r.get("total_share", 0) or 0)  # 万股
                close = float(r.get("close", 0) or 0)
                # mcap(亿) = total_share(万股) * close / 10000
                mcap = total_share * close / 10000 if total_share > 0 else 0
                if code in result:
                    result[code]["market_cap"] = mcap
        except Exception as e:
            print(f"[WARN] daily_basic 加载失败, mcap 过滤失效: {e}", flush=True)
        return result
    except Exception as e:
...
def main():
    ap = argparse.ArgumentParser(description="t-near-low 策略回测")
    ap.add_argument("--lookback", type=int, default=3, help="回看年数 (默认 3)")
    ap.add_argument("--days", type=int, default=30, help="最大持仓期 (默认 30)")
    ap.add_argument("--drop", type=float, default=0.70, help="5y 内最大回撤下限 (默认 0.70)")
    ap.add_argument("--drop-max", type=float, default=0.80, help="5y 内最大回撤上限 (默认 0.80)")
    ap.add_argument("--gap", type=float, default=3.0, help="距 5y low 阈值 (默认 3.0%)")
    ...
    ap.add_argument("--min-mcap", type=float, default=0, help="最小市值 (亿)")
    ap.add_argument("--exclude-st", action="store_true", help="排除 ST")
    ap.add_argument("--write-md", action="store_true", help="写 docs/backtest-near-low.md")
    args = ap.parse_args()
    ...
    print(f"=== t-near-low 策略回测 | {args.lookback}y ===")
    print(f"    命中条件: 5y 内 max_dd 跌 {args.drop*100:.0f}%-{args.drop_max*100:.0f}% + 距 5y low < {args.gap:.1f}%")
    ...
    print(f"\n扫描 {len(codes)} 只票 ({args.workers} workers)...")

    t0 = time.time()
    all_hits = []
    with ThreadPoolExecutor(max_workers=args.workers) as ex:
        futs = {ex.submit(scan_one, code, args.lookback, args.drop, args.drop_max, args.gap,
                          args.take_profit, args.stop_loss, args.days,
                          args.min_mcap, args.exclude_st, basic_map): code
                for code in codes}
        done = 0
    ...
    for h in all_hits[:20]:
        ...
        print(f"  {marker} {h['date']} {h['code']} @¥{h['close']:.2f} "
              ...
              f"gap={h['gap_pct']:.1f}% 5y_dd={h['max_dd_5y']:.0f}%")

    if args.write_md:
        out_path = ROOT / "docs" / "backtest-near-low.md"
        out_path.parent.mkdir(parents=True, exist_ok=True)
        md = [f"# t-near-low 策略回测 ({datetime.now().strftime('%Y-%m-%d')})\n\n"]
        md.append(f"> 策略: 5y 内 max_dd 跌 {args.drop*100:.0f}%-{args.drop_max*100:.0f}% + 距 5y low < {args.gap:.1f}%\n")
        md.append(f"> 回看: {args.lookback}y | 持仓 {args.days} 日\n")
        ...
        md.append(f"\n## 明细 ({len(all_hits)} 条)\n\n")
        md.append("| 日期 | 代码 | 收盘 | 持仓日 | 退出原因 | 单笔收益 | gap% | 5y_dd% |\n")
        md.append("|---|---|---|---|---|---|---|---|\n")
        for h in all_hits:
            marker = "✅" if h["ret_pct"] > 0 else "❌"
            md.append(f"| {h['date']} | {h['code']} | ¥{h['close']:.2f} | "
                     f"{h['exit_day']} | {h['exit_reason']} | "
                     f"{marker} {h['ret_pct']:+.1f}% | {h['gap_pct']:.1f}% | {h['max_dd_5y']:.0f}% |\n")
