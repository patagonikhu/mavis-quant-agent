...
#
# 用法:
#   bash tools/with_venv.sh python -m tools.sync --codes 002371
#   bash tools/with_venv.sh python3 tools/analyze_data.py
#   bash tools/with_venv.sh bash tools/pull_all.sh
#
# 等价于:
#   source .venv/bin/activate && python -m tools.sync --codes 002371
#
...
if [ $# -eq 0 ]; then
    ...
    echo "💡 提示: 不传参数进 REPL, 或传命令 (如 python -m tools.sync --codes 002371)"
    exec python3
fi
