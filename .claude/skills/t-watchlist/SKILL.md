---
name: t-watchlist
description: 已合并到 t-analyze。请使用 /t-analyze --all 替代。任何时候用户说"批量分析"、"全部扫一遍"、"watchlist 分析"触发时，直接调用 t-analyze skill 并传入 --all 参数。
user-invocable: true
allowed-tools:
...
> /t-watchlist                  /t-watchlist --window N       /t-watchlist --no-news        ```

#
##
> **违反 = 报告不合格** (用户已连续两次反馈)

1. **5 方法 × 3 周期 矩阵 必须在最前** (一等公民, 跟 CLAUDE.md 红字规则一致)
2. **缠论三要素** (中枢+背驰+止跌) — 不可漏
3. **4 个补充策略** (SMC-OB+量价+威科夫+多市场共振) — 不可漏
4. **PEG/DCF L** 必须在缠论 + 4 补充策略**之后**输出 (不能放最前)
5. **必须用并发** (`refresh_all.sh` 4-worker), 不允许串行 fallback
6. **场景+共振+行动** 三件套必须输出 (从 `AnalysisData.from_raw(dump).analysis` 抓: `analysis.get("scene")` / `analysis.get("resonance")` / `analysis.get("wyckoff")`)

##
- `CLAUDE.md` (决策矩阵 + 输出格式)
- `docs/analysis-framework.md` (四问 + 龙头 + T 框架 + PEG)
- `data/watchlist.json` (**必读**, 全部条目 = 候选批量列表)
- `data/events.json` (T 位置)

##
默认全跑 watchlist 所有股票. `--window N` 只跑 `T ∈ [-N, +N]` 月内的股票:

```
例: /t-watchlist --window 3
  → 只跑 T-3 ~ T+3 月窗口内的股票 (甜蜜窗口 + 顶部信号)
  → 其他股票跳过 (节省 Token)
```

> 💡 **批量持仓 / 板块分析用 t-batch (新)**
>
> - `/t-batch 300274 600089` - 6 只持仓批量分析
> - `/t-batch --sector CPO` - 板块扫描
> - `/t-batch --holdings` - 默认 6 只持仓
> - `/t-batch --all` - watchlist 全部
>
> t-batch 走 t-pull (in-process AgentData) + t-analyze (render) + 提 5 方法 × 3 周期 section
> 跟 t-watchlist 区别: t-batch 提**现成 5 方法矩阵 section**, 0 重新拼

##
**v3.2 新 (2026-07-22): 用 refresh_all 一键刷全 watchlist, 不再每只 curl!**

```bash
bash tools/refresh_all.sh
```

**为什么 refresh_all 不是 ensure_fresh:**
- `ensure_fresh.py --watchlist` 只刷 JSON, 不渲染报告
- `refresh_all.sh` 刷 JSON + 重渲染报告 (更完整)
- /t-watchlist 之后用户要马上看报告, 数据+报告都得新

**老版 (v9.1, 废弃, 不要再用):**
对每只候选股票, 跑完整 `/t-analyze {code}` 流程 (v9.1 完整版):

1. **读 dump 数据 (股价 + EPS 预测)** (先跑 t-pull, 见 t-analyze §2b/2c)
2. **Bash 拉取 K-line, 计算 MA5/20/60/120** (v5 强制, 见 t-analyze §2f)
3. **Bash 计算 PEG 四件套 (A派+C派+真实+表观)** (v4 强制, 见 t-analyze §2d)
4. **Bash 计算 DCF L (r=8/10/12% + 校正)** (见 t-analyze §2e)
5. **Bash 计算 5维技术指标 (EMA/ADX/RSI/BB/量价)** (v7, 见 t-analyze §2f.1)
6. **v9.1 板块-aware DCF 矩阵** (查 sectors.json + sector_assumptions.py, 见 t-analyze §2i)
7. WebSearch (默认开启)
8. 写 `docs/analyze-{code}-{name}.md` (含 5 方法 × 3 周期矩阵的"行动"信号, 这是实战唯一信号源)
9. 更新 watchlist (notes, 可选追加本次分析摘要)
10. 输出 60 行 t-analyze 报告 (chat)

> **v10+ 改动**: 2026-08-13 删 watchlist `rating` + `rating_history` 字段 (114 处),
> 实战不再用"长期估值评级" (滞后 27-45 天), 改用 report 5 方法矩阵的"行动"信号 (🥇/🟢/⬜/🟡/🔴).

**与 t-analyze 完全对齐，任何 t-analyze 包含的模块，t-watchlist 也必须包含：

###
每只股票分析报告**必须包含**三层仓位策略，具体价位全部基于实时数据计算：
...
【中仓 30%】
  买入信号：日线底背驰 + 缩量(vol<0.85x) + 长下影(>40%) [3个同时]
  买入价：¥{信号触发次日，预期区间下沿~上沿}
  止损：¥{日线中枢下沿}
  目标：¥{日线中枢上沿}

【波动仓 30%】
  买入：60分钟底背驰触发
  减仓：MA20偏离>30% (¥{MA20×1.3}) 或 日线顶背驰
  止损：¥{60分中枢下沿}
...
**计算方法：** 直接调用 §2m 中的 `build_strategy()` 函数，输入当前中枢价位即可。
**
- PEG 四件套 (v4): A派/C派/真实/表观 全显示，失真标⚠️
- MA 均线 (v5): MA5/20/60/120 + 排列状态 + 拉高出货检测
- 5维技术指标 (v7): 复合评分 -5~+5
- v8 股价预测: 3场景+概率+加权预期（用户问"涨多少"时触发）
- v9.1 板块-aware DCF: WACC×g 矩阵

**dump 缺字段时:** 标注 `(LLM估算，低置信度)`，DCF L / MA 不输出。

##
依次输出每只股票的 t-analyze 报告, 中间用分隔符:

```
============================================================
[完整 60 行 t-analyze 报告]
============================================================
[完整 60 行 t-analyze 报告]
============================================================
...
...
##
```
📊 批量完成: N 只 (跳过 M 只 --window 过滤)
✅ 已写入 docs/analyze-{code}.md (N 个)
✅ 已更新 watchlist (N 只 notes 追加)

💡 建议下一步:
  - 跑 /t-monitor 拉事件 + 高亮建仓/减仓窗口
  - 重新跑 /t-watchlist --window 3 (T 窗口内)
```

#
**t-watchlist = t-analyze × N (批量版本)**

| 维度 | /t-analyze | /t-watchlist |
|---|---|---|
| 标的数 | 1 只 | N 只 (全部 watchlist) |
| 输出 | 60 行报告 | N × 60 行报告 (依次) |
| 写 MD | 1 个 analyze-{code}.md | N 个 analyze-{code}.md |
| 更新 watchlist | 1 只 | N 只 |
| Token | ~5K | ~5K × N |

**何时用哪个:**
- 单只深挖 → /t-analyze {code}
- 批量刷新 (复盘日) → /t-watchlist
- 只想看清单摘要 → /t-chain {industry} (不刷新)

#
- 默认跑全部 watchlist (12-20 只), 大约 60-100K Token
- 用 `--window N` 限制只跑 T 窗口内的 (甜蜜窗口 + 顶部信号)
- WebSearch 默认开启 (单只 1-3 次调用, N 只 = N × 1-3 次)
- 不修改 events.json (新事件 y/N 确认才加)
- 报告控制在 60 行/只 (沿用 t-analyze 标准)
---
...
##
**量价分析读 data.analysis['volume_price'] (7-30 改, 不再自行实现 OBV):**

```python
# 不要自己重新实现 OBV/量价计算，读 dump 的 analysis 字段即可:
from tools.analysis.analysis_data import AnalysisData
import json
with open(f'data/dump/{code}.json') as f:
    dump = json.load(f)
data = AnalysisData.from_raw(dump)
vp = (data.analysis or {}).get('volume_price', {})
# vp 字段: obv_trend / vol_ratio / d120 / score / verdict / signals
score   = vp.get('score', 0)
verdict = vp.get('verdict', '⬜中性')
signals = vp.get('signals', [])
d120    = vp.get('d120', 0)
vr      = vp.get('vol_ratio', 1.0)
# 输出格式保持不变，数据来源改为 dump 派生
```
...
####
> **MACD 背驰必须覆盖 4 个级别，缺一不可。**
> 用户原话：「以后报告里面要看到各个级别的背驰信号」

| 级别 | 用途 | 有效期 | 数据源 | K线参数 |
|---|---|---|---|---|
| **60分钟** | 波段进出 | 6h-2d | 新浪 `scale=60` | 200 根 |
| **日线** | 子浪顶底 | 1-3 周 | 腾讯 `param=...,day` | 500 根 |
| **周线** | 主升浪方向 | 1-3 月 | 腾讯 `param=...,week` | 250 根 |
| **月线** | 长期趋势 | 3-12 月 | 腾讯 `param=...,month` | 120 根 |

**每个级别必须输出**: 面积比 (X.XX → Y.YY = ZZ%) + 5 种信号状态之一 + 操作建议
...
- 日+周 2 级顶背驰 → 🟠 减仓 1/2
- 周线单级别顶背驰 → 🟡 减仓 1/3
- 2-4 级底背驰共振 → 🟢 加仓候选 (信号强度按级别数)
- 均无 → ✅ 正常持有

**失效场景**:
- 半导体设备 / AI 芯片 / AI 服务器: 4 级背驰失效，改用 MA20 偏离 > 30%
- CPO / 光学 / 半导体封测: 4 级背驰全部有效
- 其他板块: 参考 CLAUDE.md §5.2

####
> **中枢必须覆盖 3 个级别，缺一不可。**
> 用户原话：「还有各个级别的中枢，这个也要固化，强制出现在结果里」

| 级别 | 对应仓位 | 用途 | K线参数 |
|---|---|---|---|
| **周线中枢** | 底仓 | 主升浪方向 + 长期持仓依据 | 250 根 (周) |
| **日线中枢** | 中仓 | 子浪位置 + 中线加减仓 | 500 根 (日) |
| **60分中枢** | 波动仓 | 短线进出 + 精确进场 | 200 根 (60min) |

**每个级别必须输出**:
- **下沿价格 ¥XXX** (具体数字)
- **上沿价格 ¥XXX** (具体数字)
- **当前价 vs 中枢位置**: 上方✅ / 内部⬜ / 下方⚠️
- **止损基准** (价格在中枢上方) 或 **目标位** (价格在中枢下方)
- 中枢宽度百分比
- 中枢重心趋势: 🟢上移 / ⬜横盘 / 🔴下移

**九种组合判断 (强制输出)** — 参考 CLAUDE.md §5.5 完整表:
- 周线×日线×60分 位置组合 → 状态 → 操作建议
- 至少要识别 9 种组合中当前属于哪一种
- 当前位置 + 状态 + 操作 = 三个必填字段

详见 t-analyze/SKILL.md §2m 完整函数实现 (run_beichi_4levels + run_hub_3levels)。
